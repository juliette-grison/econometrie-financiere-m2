...
# Prévision de la VaR (2023)

## Calcul de la VaR

```{r}
show(creturn[450,])	# 2019-12-31	0.002946022
b <- nrow(return)	# b=1258
estim <- 1006		# nbre observations des rentabilités de 2016 à 2019
h <- b-1007+1		# nbre observations des rentabilités en 2020
original <- return[1007:1258, 1]		# rentabilités originales en 2020
```
...
write(t(foremat),file="forecast_igarch.out",ncolumn=1,append=FALSE)
write(t(varmat),file="var_igarch.out",ncolumn=1,append=FALSE)			
```

```{r}
# Kupiec backtesting test with GAS package
alpha = 0.05
var = ts(varmat)
backtest <- BacktestVaR(data = original, VaR = var, alpha = alpha)
kupiec <- backtest$LRuc		# Kupiec test
show(kupiec)
```
